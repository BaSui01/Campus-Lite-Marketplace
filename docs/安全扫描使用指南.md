...
> 📌 **内置基线**：`backend/pom.xml` 已配置 `org.owasp:dependency-check-maven:12.1.0`，默认生成 HTML+JSON 报告，CVSS ≥ 7 自动失败；抑制规则集中在 `backend/.owasp/suppressions.xml`。
...
**执行流程（`.github/workflows/backend-quality-gates.yml`）**：
1. 启动 PostgreSQL 16 / Redis 7 容器，准备真实依赖环境。
2. `mvn -B -f backend/pom.xml -DskipITs=true test`（Surefire 单元测试）。
3. `mvn -B -f backend/pom.xml verify`（Failsafe 集成测试 + JaCoCo Coverage 校验）。
4. `mvn -B -f backend/pom.xml org.owasp:dependency-check-maven:check -Dformat=HTML -DfailBuildOnCVSS=7`（HTML 报告，CVSS≥7 直接失败）。
5. 上传 `backend/target/dependency-check-report.html` 到 Actions Artifact。
6. `mvn -B -f backend/pom.xml org.pitest:pitest-maven:mutationCoverage`（PIT 突变测试）。
7. 若 `RUN_API_E2E` / `RUN_JMETER` 置为 `true`，继续执行 Newman API 冒烟与 JMeter 压测。
...
- [前端Monorepo架构搭建完成报告](前端Monorepo架构搭建完成报告.md)（了解共享 API 客户端更新流程）
- [生产环境部署指南](生产环境部署指南.md)（部署前安全检查清单）
