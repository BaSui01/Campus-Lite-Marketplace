...
- [x] 测试：命中/误报场景、降级与人工审核队列

### [x] T8 限流与反滥用（需求 60）
 - [x] 注解 + AOP + Redis 滑动窗口/令牌桶实现（已完成：滑动窗口+令牌桶）
...
- [x] 任务查询/取消/清理接口，审计日志
- [-] 集成测试：已覆盖下载有效期/权限/触发；阈值超限路径已补（单测），大数据量 IT 待补

### [x] T10 统一任务框架（需求 62）
- [x] 任务注册/记录/指标，Redis 分布式锁
- [x] 手动触发/暂停/参数化运行
- [x] 内置任务：热榜刷新/订单超时/对账/清理导出
- [-] 测试：已覆盖成功路径、分布式锁及锁繁忙失败路径；失败重试待补
